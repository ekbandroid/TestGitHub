...
interface FavoriteRepositoriesDao {
    ...
    @Query("DELETE FROM ${FavoriteRepositoryEntity.TABLE_NAME} WHERE ${FavoriteRepositoryEntity.COLUMN_ID}=:id")
    fun delete(id: String)

    @Query("SELECT * FROM ${FavoriteRepositoryEntity.TABLE_NAME} WHERE ${FavoriteRepositoryEntity.COLUMN_ID}=:id")
